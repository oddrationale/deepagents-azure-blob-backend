# PLAN-2026-03-06.md — `deepagents-azure-blob-backend`
...
│   ├── unit_tests/
│   │   └── test_backend.py      # Unit tests (mocked, no I/O)
│   └── integration_tests/
│       ├── test_backend.py      # Integration tests (requires Azurite)
│       └── test_backend_contract.py  # BackendProtocol-style contract tests
├── examples/
│   ├── basic_agent.py
...
- `libs/deepagents/tests/unit_tests/backends/test_composite_backend.py` — best reference; covers `ls`, `read`, `write`, `edit`, `glob`, `grep` end-to-end with assertions on return shapes
...
**What your `tests/integration_tests/test_backend.py` should cover** (mirror the contract implied by the upstream tests):

| Method | Key assertions |
|--------|---------------|
| `write` / `awrite` | Returns `WriteResult(error=None, path=<path>)` |
| `read` / `aread` | Returns line-numbered string; respects `offset` and `limit` |
| `ls` / `als` | Returns `LsResult`; directories have trailing `/` and `is_dir=True`; non-existent path returns empty entries |
| `edit` / `aedit` | Returns `EditResult(error=None, ...)`; fails if `old_str` not unique |
| `glob` / `aglob` | Supports `*`, `**`, `?` patterns |
| `grep` / `agrep` | Returns `GrepResult` with correct `path`, `line`, `text` |
| `upload_files` / `download_files` | Round-trip bytes fidelity |

Also add `tests/unit_tests/test_backend.py` that mocks the Azure SDK client (`AsyncMock` on `BlobServiceClient`) to cover error paths (blob not found, network failure, encoding errors) without needing Azurite.
